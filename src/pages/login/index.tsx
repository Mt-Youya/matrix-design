import React, { useRef, useEffect, useContext } from "react"
import { HomeContext } from "../home"
import { request, METHOD } from "@/utils/request"
import "./index.modules.less"

const Login = () => {
  const aContainer = useRef<HTMLDivElement | null>(null)
  const bContainer = useRef<HTMLDivElement | null>(null)
  const switchCtn = useRef<HTMLDivElement | null>(null)
  const switchC1 = useRef<HTMLDivElement | null>(null)
  const switchC2 = useRef<HTMLDivElement | null>(null)
  const switchCircle1 = useRef<HTMLDivElement | null>(null)
  const switchCircle2 = useRef<HTMLDivElement | null>(null)
  const signInBtn = useRef<HTMLButtonElement | null>(null)
  const signUpBtn = useRef<HTMLButtonElement | null>(null)
  const signInSubmit = useRef<HTMLButtonElement | null>(null)
  const signUpSubmit = useRef<HTMLButtonElement | null>(null)

  const ctx = useContext(HomeContext)
  console.log("ctx", ctx)

  const onSubmit = (e: any) => {
    // console.log(e)
    request("/pc/login", METHOD.POST, {
      password: "",
      sobId: "",
      url: "",
      username: ""
    }).then(res => {
      console.log(res)
    })

    e.preventDefault()
  }

  const changeForm = () => {
    switchCtn.current?.classList.add("is-gx")
    setTimeout(function () {
      switchCtn.current?.classList.remove("is-gx")
    }, 1500)

    switchCtn.current?.classList.toggle("is-txr")
    switchCircle1.current?.classList.toggle("is-txr")
    switchCircle2.current?.classList.toggle("is-txr")

    switchC1.current?.classList.toggle("is-hidden")
    switchC2.current?.classList.toggle("is-hidden")
    aContainer.current?.classList.toggle("is-txl")
    bContainer.current?.classList.toggle("is-txl")
    bContainer.current?.classList.toggle("is-z200")
  }

  useEffect(() => {
    signInSubmit.current?.addEventListener("click", onSubmit)
    signUpSubmit.current?.addEventListener("click", onSubmit)
    signInBtn.current?.addEventListener("click", changeForm)
    signUpBtn.current?.addEventListener("click", changeForm)
    return () => {
      signInSubmit.current?.removeEventListener("click", onSubmit)
      signUpSubmit.current?.removeEventListener("click", onSubmit)
      signInBtn.current?.removeEventListener("click", changeForm)
      signUpBtn.current?.removeEventListener("click", changeForm)
    }
  })

  return (
    <div className="login-container">
      <div className="login-main">
        <div className="container a-container" id="a-container" ref={aContainer}>
          <form className="form" id="a-form" method="" action="" onSubmit={onSubmit}>
            <h2 className="form_title title">Create Account</h2>
            <div className="form__icons">
              <img
                className="form__icon"
                src="data:image/svg+xml;base64,"
                alt=""
              />
              <img
                className="form__icon"
                src="data:image/svg+xml;base64,"
              />
              <img
                className="form__icon"
                src="data:image/svg+xml;base64,"
              />
            </div>
            <span className="form__span">or use email for registration</span>
            <input className="form__input" type="text" placeholder="Name" />
            <input className="form__input" type="text" placeholder="Email" />
            <input className="form__input" type="password" placeholder="Password" />
            <button className="form__button button submit" type="submit">
              SIGN UP
            </button>
          </form>
        </div>
        <div className="container b-container" id="b-container" ref={bContainer}>
          <form className="form" id="b-form" method="" action="" onSubmit={onSubmit}>
            <h2 className="form_title title">Sign in to Website</h2>
            <div className="form__icons"></div>
            <span className="form__span">or use your email account</span>
            <input className="form__input" type="text" placeholder="Email" />
            <input className="form__input" type="password" placeholder="Password" />
            <a className="form__link">Forgot your password?</a>
            <button className="form__button button submit" type="submit">
              SIGN IN
            </button>
          </form>
        </div>
        <div className="switch" id="switch-cnt" ref={switchCtn}>
          <div className="switch__circle" ref={switchCircle1}></div>
          <div className="switch__circle switch__circle--t" ref={switchCircle2}></div>
          <div className="switch__container" id="switch-c1" ref={switchC1}>
            <h2 className="switch__title title">Hello Friend !</h2>
            <p className="switch__description description">
              To keep connected with us please login with your personal info
            </p>
            <button className="switch__button button switch-btn" ref={signInBtn}>
              SIGN IN
            </button>
          </div>
          <div className="switch__container is-hidden" id="switch-c2" ref={switchC2}>
            <h2 className="switch__title title">Welcome Back !</h2>
            <p className="switch__description description">
              Enter your personal details and start journey with us
            </p>
            <button className="switch__button button switch-btn" ref={signUpBtn}>
              SIGN UP
            </button>
          </div>
        </div>
      </div>
    </div>
  )
}

export default Login
